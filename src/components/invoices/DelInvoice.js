...
import { deleteInv } from '../../actions/index'
...
class DelCust extends Component {
  ...
  handleDelete(e) {
    const { invoices, invoice } = this.props
    let invoicesList = invoices.filter(item => item.id != invoice.id )
    this.props.dispatch(deleteInv(invoicesList))
    this.close();
  }
  render() {
    return (
